...
#region Inspector Variables
	...
	[HideInInspector]
	public bool flexibleDistance = true;
	...
	[HideInInspector]
	public float maximumDistance = 15.0f;
#endregion
	...
	private void Rotate()
	{
			...
			case ControlType.Mouse:
				{
					...
					xDelta = Input.GetAxis("Mouse X") * 30;
					yDelta = Input.GetAxis("Mouse Y") * 30;
					//*/
				}
				break;
		...
		if (xDelta != 0 || yDelta != 0)
		{
			...
			rotationX = xRotation ? rotationX : 0.0f;
			rotationY = yRotation ? rotationY : 0.0f;
	...
	private void FlexibleDistance()
	{
		...
		if (Physics.Raycast(origin, dir, out RaycastHit hit, originalOffset.magnitude, obstacleLayer))
		{
		...
		else
		{
			...
			Debug.DrawRay(origin, dir.normalized * originalOffset.magnitude, Color.green);
		}
	}
	...
	private void Zoom(float amount)
	{
		if (!zoom || 
			(amount == 0) ||
			((originalOffset.magnitude <= minimumDistance) && (amount > 0)) ||
			((originalOffset.magnitude >= maximumDistance) && (amount < 0)))
		{
			return;
		}

		float distance = originalOffset.magnitude - amount;
		float rate = distance / originalOffset.magnitude;
		originalOffset *= rate;
	}
